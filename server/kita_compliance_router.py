...
# === WORLD-ROUTER ===
"""
SHIKSHA · shiksha.world Router
Liefert die Plattform-Site und die Live-Daten für sie.

Endpoints:
  GET  /m/shiksha                    — die Plattform-Site
  GET  /api/platform/modules         — Modul-Showcase (live)
  GET  /api/platform/live-stats      — anonymisierte Live-Daten
  GET  /api/platform/greeting        — Tageszeit-Anrede für die Hero

Anhängen an /opt/shiksha/kita_compliance_router.py.
"""

import json as _ws_json
import datetime as _ws_dt
from datetime import date as _ws_date

from fastapi.responses import HTMLResponse


def _ws_season(today=None):
    today = today or _ws_date.today()
    m = today.month
    if m in (3, 4, 5):    return "spring"
    if m in (6, 7, 8):    return "summer"
    if m in (9, 10, 11):  return "autumn"
...
@kita_router.get("/api/platform/modules")
...
async def platform_live_stats():
    ...
    try:
        with engine.connect() as conn:
            ...
            try:
                stats["events_this_week"] = int(conn.execute(sa.text("""
                    SELECT COUNT(*) FROM kita_calendar_events
                    WHERE start_date BETWEEN :a AND :b
                """), {"a": today - _ws_dt.timedelta(days=today.weekday()),
                       "b": today + _ws_dt.timedelta(days=6 - today.weekday())}).scalar() or 0)
            except Exception: pass
            # Module live
        ...
        stats["error"] = str(e)
    stats["timestamp"] = _ws_dt.datetime.now().isoformat()
    return stats


@kita_router.get("/api/platform/greeting")
async def platform_greeting():
    """Lebendige Hero-Begrüßung für shiksha.world — datenschutz-freundlich."""
    now = _ws_dt.datetime.now()
    h = now.hour
...
@kita_router.get("/m/shiksha", response_class=HTMLResponse)
async def platform_site():
    """Die Plattform-Site selbst — vollständig durch das Template gerendert."""
    import pathlib as _ws_path
    tmpl_path = _ws_path.Path("/opt/shiksha/marketing_templates/platform_meta.html")
